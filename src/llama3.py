...
SYSTEM_PROMPT="""\
You are Llama3, an AI assistant with expertise in converting textual problem
...
- For the Python code using the PulP library, use x >= y + 1 instead of x > y.
...
def write_to_markdown(problem_id, question, formulation, code):
    with open(f"data/{DATASET}/solutions_llama3/generation.md", "a") as file:
        file.write(f"# id\n{problem_id}\n\n")
...
def save_problem_results(problem, problem_id):
    ...
    os.makedirs(f"data/{DATASET}/problems_llama3", exist_ok=True)

    # Save the problem with its results to a separate file
    filename = f"data/{DATASET}/problems_llama3/prediction_{problem_id:03d}.json"
...
def main():
    ...
    for i, problem in enumerate(data):
        ...
        prediction_file = f"data/{DATASET}/problems_llama3/prediction_{i:03d}.json"
