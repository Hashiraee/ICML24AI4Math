...
SYSTEM_PROMPT="""\
You are Claude, an AI assistant with expertise in converting textual problem
...
- For problems with "number of acres", use continuous variables instead of integer variables.
...
def send_to_llm(prompt):
    ...
    assistant_response = " ".join(block.text for block in content_blocks)
...
def write_to_markdown(problem_id, question, formulation, code):
    with open(f"data/{DATASET}/solutions_opus/generation.md", "a") as file:
        file.write(f"# id\n{problem_id}\n\n")
...
def save_problem_results(problem, problem_id):
    ...
    os.makedirs(f"data/{DATASET}/problems_opus", exist_ok=True)

    # Save the problem with its results to a separate file
    filename = f"data/{DATASET}/problems_opus/prediction_{problem_id:03d}.json"
...
def main():
    ...
    for i, problem in enumerate(data):
        ...
        prediction_file = f"data/{DATASET}/problems_opus/prediction_{i:03d}.json"
