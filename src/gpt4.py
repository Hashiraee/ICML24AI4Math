...
def write_to_markdown(problem_id, question, formulation, code):
    with open(f"data/{DATASET}/solutions_gpt4/generation.md", "a") as file:
        file.write(f"# id\n{problem_id}\n\n")
...
def save_problem_results(problem, problem_id):
    ...
    os.makedirs(f"data/{DATASET}/problems_gpt4", exist_ok=True)

    # Save the problem with its results to a separate file
    filename = f"data/{DATASET}/problems_gpt4/prediction_{problem_id:03d}.json"
...
def main():
    ...
    for i, problem in enumerate(data):
        ...
        prediction_file = f"data/{DATASET}/problems_gpt4/prediction_{i:03d}.json"
